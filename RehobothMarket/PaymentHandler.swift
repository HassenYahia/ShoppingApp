...
//  RehobothMarket
//
//  Created by Hassen Abdurahman Yahia on 2023-08-13.
//
...
class PaymentHandler: NSObject {
    ...
    func shippingMethodCalculator() -> [PKShippingMethod] {
        ...
        if let shippingEnd = shippingEnd, let shippingStart = shippingStart {
            ...
            let shipingDelivery = PKShippingMethod(label: "Delivery", amount: NSDecimalNumber(string: "0.00"))
            shipingDelivery.dateComponentsRange = PKDateComponentsRange(start: startComponents, end: endComponents)
            shipingDelivery.detail = "Order sent to your address"
            shipingDelivery.identifier = "DELIVERY"
            
            return [shipingDelivery]
        }
        return []
    }
    ...
    func startPayment(products: [Product], total: Int, completion: @escaping PaymentCompletionHandler) {
        ...
        paymentRequest.merchantIdentifier = "merchant.com.rehobothcafeandmarket.rehobothcafeandmarket"
        ...
        paymentController?.present(completion: {(presented: Bool) in
            if presented {
                ...
                debugPrint("Failed to present payment contorller")
            }
        })
    }
}
